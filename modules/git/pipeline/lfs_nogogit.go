...
type LFSResult struct {
	...
	Summary        string
	When           time.Time
	ParentHashes   []git.HashTypeInterface
	BranchName     string
...
func FindLFSFile(repo *git.Repository, hash git.HashTypeInterface) ([]*LFSResult, error) {
	...
	workingShaBuf := make([]byte, hash.FullLength()/2)
	...
	commitReadingLoop:
		for {
			...
			case "tree":
				var n int64
				for n < size {
					mode, fname, binHash, count, err := git.ParseTreeLine(hash, batchReader, modeBuf, fnameBuf, workingShaBuf)
					if err != nil {
						return nil, err
					}
					n += int64(count)
					if bytes.Equal(binHash, hash.RawValue()) {
						result := LFSResult{
						...
						hexHash := make([]byte, hash.FullLength())
						git.BinToHexHash(hash, binHash, hexHash)
						trees = append(trees, hexHash)
