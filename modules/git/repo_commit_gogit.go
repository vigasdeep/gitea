...
func (repo *Repository) ConvertToGitHash(commitID string) (Hash, error) {
	h := repo.Hash
	if len(commitID) == hash.HexSize && h.IsValid(commitID) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "unknown revision or path") ||
			...
			return h.Empty(), ErrNotExist{commitID, ""}
		}
		return h.Empty(), err
	}

	return HashFromString(actualCommitID)
}

// IsCommitExist returns true if given commit exists in current repository.
func (repo *Repository) IsCommitExist(name string) bool {
	hash, err := repo.ConvertToGitHash(name)
	if err != nil {
		return false
	}
