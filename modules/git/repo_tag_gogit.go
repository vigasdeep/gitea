...
func (repo *Repository) GetTagType(id HashTypeInterface) (string, error) {
	// Get tag type
...
func (repo *Repository) getTag(tagID HashTypeInterface, name string) (*Tag, error) {
	...
	commitID, err := HashTypeInterfaceFromHashString(commitIDStr)
	if err != nil {
		return nil, err
	}
	...
	tag := &Tag{
		Name:    name,
		ID:      tagID,
		Object:  commitID.MustID(gogitTag.Target[:]),
		Type:    tp,
