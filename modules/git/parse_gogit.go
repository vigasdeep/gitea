...
func ParseTreeEntries(h HashType, data []byte) ([]*TreeEntry, error) {
...
func parseTreeEntries(data []byte, ptree *Tree) ([]*TreeEntry, error) {
	...
	for pos := 0; pos < len(data); {
		...
		var err error
		entry.ID, err = HashFromString(string(data[pos : pos+hash.Size*2]))
		if err != nil {
			return nil, fmt.Errorf("invalid ls-tree hash: %s", string(data))
		}
