...
func (t *Tree) ListEntries() (Entries, error) {
	...
	if t.repo != nil {
		...
		if typ == "tree" {
			t.entries, err = catBatchParseTreeEntries(t.ID, t, rd, sz)
			if err != nil {
				return nil, err
			}
	...
	var err error
	t.entries, err = parseTreeEntries(t.repo.Hash, stdout, t)
	if err == nil {
		t.entriesParsed = true
	}
...
func (t *Tree) listEntriesRecursive(extraArgs TrustedCmdArgs) (Entries, error) {
	...
	var err error
	t.entriesRecursive, err = parseTreeEntries(t.repo.Hash, stdout, t)
	if err == nil {
