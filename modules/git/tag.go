...
type Tag struct {
	Name      string
	ID        HashTypeInterface
	Object    HashTypeInterface // The id of this commit object
	Type      string
	Tagger    *Signature
	Message   string
...
func parseTagData(hash HashTypeInterface, data []byte) (*Tag, error) {
	tag := new(Tag)
	tag.ID = hash.NewEmptyID()
	tag.Object = hash.NewEmptyID()
...
l:
	for {
		...
		case eol > 0:
			...
			case "object":
				id, err := hash.NewIDFromString(string(line[spacepos+1:]))
				if err != nil {
					return nil, err
				}
				tag.Object = id
