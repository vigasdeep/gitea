...
func (repo *Repository) GetTagInfos(page, pageSize int) ([]*Tag, int, error) {
	...
	for {
		...
		tag, err := parseTagRef(repo.Hash, ref)
		if err != nil {
...
func parseTagRef(hash HashTypeInterface, ref map[string]string) (tag *Tag, err error) {
	tag = &Tag{
		Type: ref["objecttype"],
		Name: ref["refname:short"],
	}

	tag.ID, err = hash.NewIDFromString(ref["objectname"])
	if err != nil {
	...
	if tag.Type == "commit" {
		...
		// annotated tag
		tag.Object, err = hash.NewIDFromString(ref["object"])
		if err != nil {
...
func (repo *Repository) GetAnnotatedTag(sha string) (*Tag, error) {
	id, err := repo.Hash.NewIDFromString(sha)
	if err != nil {
		return nil, err
	}
