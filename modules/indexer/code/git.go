...
func parseGitLsTreeOutput(hash git.HashTypeInterface, stdout []byte) ([]internal.FileUpdate, error) {
	entries, err := git.ParseTreeEntries(hash, stdout)
	if err != nil {
		return nil, err
	}
	idxCount := 0
...
func genesisChanges(ctx context.Context, repo *repo_model.Repository, revision string) (*internal.RepoChanges, error) {
	...
	changes.Updates, err = parseGitLsTreeOutput(hash, stdout)
	return &changes, err
}
...
func nonGenesisChanges(ctx context.Context, repo *repo_model.Repository, revision string) (*internal.RepoChanges, error) {
	...
	changes.Updates, err = parseGitLsTreeOutput(hash, lsTreeStdout)
	return &changes, err
}
