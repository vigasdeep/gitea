...
func preReceiveBranch(ctx *preReceiveContext, oldCommitID, newCommitID string, refFullName git.RefName) {
	...
	if branchName == repo.DefaultBranch && newCommitID == gitRepo.Hash.Empty().String() {
	...
	if newCommitID == gitRepo.Hash.Empty().String() {
	...
	if oldCommitID != gitRepo.Hash.Empty().String() {
