...
func ConvertToSHA1(ctx gocontext.Context, repo *context.Repository, commitID string) (git.HashTypeInterface, error) {
	hash := repo.GitRepo.Hash
	if len(commitID) == hash.FullLength() && hash.IsValid(commitID) {
		sha, err := hash.NewIDFromString(commitID)
		if err == nil {
			return sha, nil
		}
	}
	...
	if err != nil {
		return hash.NewEmptyID(), fmt.Errorf("RepositoryFromContextOrOpen: %w", err)
	}
	defer closer.Close()

	return gitRepo.ConvertToGitHash(commitID)
}
