...
func pushUpdates(optsList []*repo_module.PushUpdateOptions) error {
	...
	hash, err := gitRepo.HashTypeInterface()
	if err != nil {
		return fmt.Errorf("unknown Hash type for repository [%s]: %w", repoPath, err)
	}
	...
	for _, opts := range optsList {
		...
		if opts.IsNewRef() && opts.IsDelRef() {
			return fmt.Errorf("old and new revisions are both %s", hash.Empty())
		}
		if opts.RefFullName.IsTag() {
			...
			if opts.IsDelRef() {
				...
				commits.CompareURL = repo.ComposeCompareURL(hash.Empty(), opts.NewCommitID)

				notify_service.PushCommits(
					ctx, pusher, repo,
					&repo_module.PushUpdateOptions{
						RefFullName: opts.RefFullName,
						OldCommitID: hash.Empty(),
			...
			if !opts.IsDelRef() {
				...
				oldCommitID := opts.OldCommitID
				if oldCommitID == hash.Empty() && len(commits.Commits) > 0 {
				...
				if oldCommitID == hash.Empty() && repo.DefaultBranch != branch {
					oldCommitID = repo.DefaultBranch
				}

				if oldCommitID != hash.Empty() {
