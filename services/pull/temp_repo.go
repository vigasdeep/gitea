...
func createTemporaryRepoForPR(ctx context.Context, pr *issues_model.PullRequest) (prCtx *prContext, cancel context.CancelFunc, err error) {
	...
	hash, err := git.GetHashTypeOfRepo(ctx, baseRepoPath)
	if err != nil {
		log.Error("Unable to fetch hash of repository %s: %v", baseRepoPath, err)
		cancel()
		return nil, nil, err
	}

	if err := git.InitRepository(ctx, tmpBasePath, false, hash.ToTypeString()); err != nil {
	...
	} else if len(pr.HeadCommitID) == hash.FullLength() { // for not created pull request
