...
func GetDiff(gitRepo *git.Repository, opts *DiffOptions, files ...string) (*Diff, error) {
	...
	if (len(opts.BeforeCommitID) == 0 || opts.BeforeCommitID == gitRepo.Hash.Empty()) && commit.ParentCount() == 0 {
		cmdDiff.AddArguments("diff", "--src-prefix=\\a/", "--dst-prefix=\\b/", "-M").
			AddArguments(opts.WhitespaceBehavior...).
			AddDynamicArguments(gitRepo.Hash.EmptyTree()).
	...
	if len(opts.BeforeCommitID) == 0 || opts.BeforeCommitID == gitRepo.Hash.Empty() {
		diffPaths = []string{gitRepo.Hash.EmptyTree(), opts.AfterCommitID}
	}
